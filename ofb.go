...
func encryptOFB(block cipher.Block, src, iv []byte, pad padding.Padding) ([]byte, error) {
	var err error
	if pad != nil {
...
func decryptOFB(block cipher.Block, src, iv []byte, pad padding.Padding) ([]byte, error) {
	dst := make([]byte, len(src))
	cipher.NewOFB(block, iv).XORKeyStream(dst, src)
	if pad != nil {
